    ...
    validate(state, thrones) match
      ...
      case None =>
        for
          ...
          newFeatures = resolved.map { case (province, fid) => ProvinceFeature(province, fid) }
          updatedState = state.copy(terrains = updatedTerrains, features = existingFeatures ++ newFeatures)
