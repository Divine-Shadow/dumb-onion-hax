...
class MapWrapWorkflowImpl(
  ...
  override def run(cfg: PathsConfig): IO[Either[Throwable, Unit]] =
    ...
    for
      ...
      _ <- updateCheck match
        case Right(UpdateStatus.UpdateAvailable) => IO.println("A new version of the app is available.")
      ...
      result <- latestEC.traverse { latest =>
        val targetDir = destRoot / latest.fileName.toString
        for
          ...
          updatedEC <- throneService.update[ErrorOr](layer.state, settings.thrones.placements)
          updatedState <- IO.fromEither(updatedEC)
